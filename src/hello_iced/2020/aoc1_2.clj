...
(def input (map (fn [x] (parse-long x)) (str/split-lines (slurp "resources/2020/aoc1_2.input"))))


(comment 
  (->> (for [a input b input c input] [a b c])
       (map (fn [[x y z]] (vector (+ x y z) x y z)))
       (filter (fn [[sum]] (= 2020 sum)))
       (map (fn [[_ x y z]] (* x y z)))
       first))
